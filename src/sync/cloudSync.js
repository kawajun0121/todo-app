/*
 ...
 - 【並び順をidで固定する理由】マージ結果の配列の並び順を「ローカル配列の元の並び＋新規追加分を
   後ろに足す」だけにしていたところ、両端末で別々に新規作成したTODOがある場合、各端末の
   ローカル配列の並び順（＝オブジェクトのキー挿入順）がそもそも端末ごとに異なってしまい、
   中身は全く同じセットなのに「配列の並び順が違う」という理由だけでsameItems()がfalseになり、
   両端末が絶えずお互いに書き込み→相手の受信→また書き込み…を無限に繰り返す不具合があった
   （PCとiPhoneを同時に開いていると常にガクガクする不具合の原因）。idで並べ替えてから比較・保存
   することで、内容が同じなら両端末が必ず同じ配列（＝同じJSON文字列）になるようにし、この
   無限ループを断つ。
...
(function (App) {
  ...
  function mergeById(localItems, remoteItems, timeField) {
    ...
    // idで固定順にする（理由はファイル冒頭コメントの「並び順をidで固定する理由」参照）。
    // 画面上の表示順はorderフィールドや日付など別のロジックで決まるため、ここでの並び替えは無関係。
